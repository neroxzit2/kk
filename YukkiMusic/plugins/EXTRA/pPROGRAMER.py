...
@app.on_message(
    command(["مبرمج","مطور السورس","المبرمج"])
    & filters.group
  
)
async def yas(client, message):
    usr = await client.get_chat("TO STETCHFY")
    name = usr.first_name
    photo = await app.download_media(usr.photo.big_file_id)
    await message.reply_photo(photo,       caption=f"**⩹━★⊷⌯𝐒𝐎𝐔𝐑𝐂𝐄 STETCHFY⌯⊶★━⩺\n\n⭐ ¦𝚅𝙴𝙶𝙰 :{name}\n🎯 ¦𝚄𝚂𝙴𝚁 :@{usr.username}\n🎃 ¦𝙸𝙳 :`{usr.id}`\n💌 ¦𝙱𝙸𝙾 :{usr.bio}\n\n**⩹━★⊷⌯𝐒𝐎𝐔𝐑𝐂𝐄 STETCHFY⌯⊶★━⩺**",  
...
async def kimmyy(client: Client, message: Message):
    ...
    async for photo in client.iter_profile_photos(OWNER, limit=1):
                    await message.reply_photo(photo.file_id,       caption=f"""**⩹━★⊷⌯𝐒𝐎𝐔𝐑𝐂𝐄 STETCHFY⌯⊶★━⩺**
