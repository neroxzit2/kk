...
@app.on_message(command(["00", "تليغراف", "تلجراف", "تليجراف", "تلغراف", "تلغراف ميديا", "ميديا"]) )
async def telegraph(client: Client, message: Message):
    replied = message.reply_to_message
    if not replied:
        await message.reply("🤕 ¦ الرد على ملف وسائط مدعوم\n•حط صوره و اكتب عليها تلغراف او 00")
        return
    if not (
...
async def hshs(client: Client, message: Message):      
    ...
    async for photo in client.iter_profile_photos(message.reply_to_message.from_user.id, limit=1):
                    await message.reply_photo(photo.file_id,       caption=f"""**[★𝐒𝐎𝐔𝐑𝐂𝐄 STETCHFY ˹⚡˼⁩](https://t.me/STETCHFY)\n\n🐉 ¦ ɴᴀᴍᴇ : {name}\n🤡 ¦ ᴜѕᴇ : {username}\n🔥 ¦ ɪᴅ : `{user_id}`\n🔅 ¦ ɪᴅ ᴄʜᴀᴛ : `{chat_idd}`\n💭 ¦ ᴄʜᴀᴛ : {chat_name}\n🐊 ¦ ɢʀᴏᴜᴘ : {chat_username} \n**""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
...
@app.on_message(command(["تحويل صوره", "تحويل لصوره", "تحويل الصوره"]))
...
async def ginnj(client: Client, message: Message):
    ...
    if photo is None:
        ...
        photo = await client.download_media(photo.big_file_id)
